    ...
    return (
      ...
      >
        <div className="text-center py-12">
          <h2 className="text-2xl font-bold mb-4 text-white">Please sign in to view your applications</h2>
                    ...
                    <ul className="space-y-2">
                      {(() => {
                        ...
                        if (Array.isArray(selectedApplication.requirements)) {
                          ...
                          const reqStr: string = selectedApplication.requirements;
                          ...
                          try {
                            ...
                            if (Array.isArray(parsed)) {
                              ...
                              requirements = reqStr
                                .split('\n')
                                .filter((item: string) => item.trim() && item.trim().length > 0);
                            }
                          } catch (e) {
                        ...
                        if (requirements.length === 0 && typeof selectedApplication.requirements === 'string') {
                          const text: string = selectedApplication.requirements;
                          if (text.includes('•')) {
                            requirements = text.split('•').filter((item: string) => item.trim());
                            ...
                            requirements = text.split('.').filter((item: string) => item.trim() && item.length > 10);
                          }
                        }
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        ...
                        if (Array.isArray(selectedApplication.skills)) {
                          ...
                          const skillsStr: string = selectedApplication.skills;
                          // Try to parse as JSON first
                          try {
                        ...
                        if (skills.length === 0 && typeof selectedApplication.skills === 'string') {
                          const text: string = selectedApplication.skills;
                          if (text.includes(',')) {
                            skills = text.split(',').filter((item: string) => item.trim());
                          } else if (text.includes('•')) {
                            skills = text.split('•').filter((item: string) => item.trim());
                          } else if (text.includes('-')) {
                            skills = text.split('-').filter((item: string) => item.trim());
                            ...
                            // Last resort: split by periods
                            skills = text.split('.').filter((item: string) => item.trim() && item.length > 5);
                          }
                        }
                    ...
                    <ul className="space-y-2">
                      {(() => {
                        ...
                        if (Array.isArray(selectedApplication.benefits)) {
                          ...
                          const benefitsStr: string = selectedApplication.benefits;
                          // Try to parse as JSON first
                          try {
                        ...
                        if (benefits.length === 0 && typeof selectedApplication.benefits === 'string') {
                          const text: string = selectedApplication.benefits;
                          if (text.includes('•')) {
                            benefits = text.split('•').filter((item: string) => item.trim());
                          } else if (text.includes('-')) {
                            benefits = text.split('-').filter((item: string) => item.trim());
                          } else if (text.includes(';')) {
                            benefits = text.split(';').filter((item: string) => item.trim());
                            ...
                            // Last resort: split by periods
                            benefits = text.split('.').filter((item: string) => item.trim() && item.length > 10);
                          }
                        }
