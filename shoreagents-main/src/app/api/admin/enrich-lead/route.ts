...
interface EnrichLeadRequestBody {
  userId: string
  email: string
  name: string
  company: string
...
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as EnrichLeadRequestBody;
    ...
    let serperResponse: SerperSearchResponse = { organic: [], knowledgeGraph: {} };
    let searchQuery = '';
    ...
    for (const query of searchStrategies) {
      ...
      const response: SerperSearchResponse = await fetch('https://google.serper.dev/search', {
        method: 'POST',
        headers: {
    ...
    const companyResponse: SerperSearchResponse = await fetch('https://google.serper.dev/search', {
      method: 'POST',
      headers: {
        'X-API-KEY': serperApiKey,
        'Content-Type': 'application/json',
    ...
    const userImageResponse: SerperImageResponse = await fetch('https://google.serper.dev/images', {
      method: 'POST',
      headers: {
        'X-API-KEY': serperApiKey,
        'Content-Type': 'application/json',
    ...
    const companyLogoResponse: SerperImageResponse = await fetch('https://google.serper.dev/images', {
      method: 'POST',
      headers: {
        'X-API-KEY': serperApiKey,
        'Content-Type': 'application/json',
    ...
    const knowledgeGraph: SerperKnowledgeGraph = serperResponse.knowledgeGraph ?? {};
    
    // Parse company search results
    const companyOrganic = companyResponse.organic || [];
    const companyKnowledgeGraph: SerperKnowledgeGraph = companyResponse.knowledgeGraph ?? {};
    ...
    const linkedinResult = organicResults.find((result: SerperOrganicResult) => 
    ...
    const twitterResult = organicResults.find((result: SerperOrganicResult) => 
    ...
    const facebookResult = organicResults.find((result: SerperOrganicResult) => 
    ...
    if (!companyWebsite && companyOrganic.length > 0) {
      const firstResult = companyOrganic.find((result: SerperOrganicResult) => 
    ...
    if (!companyWebsite) {
      const websiteResult = organicResults.find((result: SerperOrganicResult) => 
        !result.link?.includes('linkedin.com') &&
        !result.link?.includes('twitter.com') &&
        !result.link?.includes('facebook.com') &&
    ...
    if (userImageResponse.images && userImageResponse.images.length > 0) {
      ...
      const linkedinImage = userImageResponse.images.find((img) => 
        img.link?.includes('linkedin.com') || img.source?.includes('linkedin')
      );
      userProfilePicture = linkedinImage?.imageUrl || userImageResponse.images[0]?.imageUrl || null;
    }
    ...
    if (companyLogoResponse.images && companyLogoResponse.images.length > 0) {
      ...
      const ownDomainLogo = companyLogoResponse.images.find((img) => 
        companyWebsite && img.link?.includes(new URL(companyWebsite).hostname)
      );
      companyLogo = ownDomainLogo?.imageUrl || companyLogoResponse.images[0]?.imageUrl || null;
    }
